...
async function main() {
  ...
  console.log("Signed up as:", uid);

  // 2. Try to update profile role to admin
  ...
  console.log("Profile update result:", profData);
  console.log("Profile update error:", profErr);

  // 3. Try to insert to debts
  const { data: insertData, error: insertError } = await supabase
    .from("debts")
    .insert({
      description: "Test Insert Admin",
